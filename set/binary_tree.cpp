...
iterator next_position(iterator p) {
	if(p->right != NULL) return left_most(p->right);
	iterator last = p;
	while(p != NULL) {
		if(p->left == last) return p;
		last = p;
		p = p->parent;
	}
	return p;
...
iterator prev_position(iterator p) {
	if(p->left != NULL) return right_most(p->left);
	iterator last = p;
	while(p != NULL) {
		if(p->right == last) return p;
		last = p;
		p = p->parent;
	}
	return p;
...
template<typename iterator>
void print_order(iterator t) {
	for(auto p = left_most(t); p != NULL; p = next_position(p)) cout << p->data << " ";
	cout << endl;
}

template<typename iterator>
void print_rorder(iterator t) {
	for(auto p = right_most(t); p != NULL; p = prev_position(p)) cout << p->data << " ";
	cout << endl;
}
